...
public class SimpleHashtable<K, V> implements Iterable<SimpleHashtable.TableEntry<K, V>> {
	...
	public void put(K key, V value) {
		...
		//I think it is better to first see if adding new node would mean
		//that we need to realocate table because in that case I would only put
		//new element once, not twice
		if(!containsKey(key) && (size + 1) >= OCCUPANCY * table.length ) {
			realocateTable();
		}
